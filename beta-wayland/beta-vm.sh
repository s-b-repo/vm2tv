...
install_packages() {
    ...
    sudo apt-get update
    if sudo apt-get install -y ffmpeg avahi-daemon nginx gstreamer1.0-tools wf-recorder; then
...
# Function to configure FFmpeg or wf-recorder HLS stream
configure_stream_hls() {
    echo "Starting stream for HLS..."

    # Ask user for screen size and Wayland/X11
    read -p "Are you using Wayland (y/n)? " wayland
    read -p "Enter the screen resolution (e.g., 1920x1080): " screen_size
    read -p "Enter the display number (default for X11: :0.0): " display_number
    display_number=${display_number:-":0.0"}

    # Ask user for bitrate
    read -p "Enter the desired video bitrate (e.g., 2000k for 2 Mbps): " bitrate
    ...
    if [ "$wayland" == "y" ]; then
        ...
        ffmpeg -f x11grab -s "$screen_size" -i "$display_number" -f pulse -ac 2 -i default \
            -c:v libx264 -b:v "$bitrate" -pix_fmt yuv420p -preset fast -crf 28 -g 50 -c:a aac \
            -f hls -hls_time 2 -hls_list_size 10 -hls_flags delete_segments "$hls_output" &
    fi
    
    if [ $? -eq 0 ]; then
        echo "Stream is running and streaming to $hls_output"
    else
        echo "Failed to start stream. Exiting."
        exit 1
    fi
}
...
# Step 3: Configure stream (FFmpeg or wf-recorder for Wayland)
configure_stream_hls
