...
check_system_resources() {
    ...
    if [ "$cpu_cores" -lt 2 ]; then
        echo "Warning: Only $cpu_cores CPU core(s). Consider 'Low' quality preset."
    fi
...
install_packages() {
    ...
    local packages=(ffmpeg avahi-daemon nginx pulseaudio-utils curl)
    if [ "$display_server" = "wayland" ]; then
        packages+=(wf-recorder pipewire pipewire-pulse slurp)
    else
        packages+=(x11-utils xdotool)
    fi
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .controls { display: flex; gap: 0.5rem; margin-top: 0.5rem; justify-content: center; flex-wrap: wrap; }
...
select_capture_source() {
    ...
    case "$capture_choice" in
        2)
            CAPTURE_MODE="window"
            if [ "$display_server" = "wayland" ]; then
                echo "On Wayland, the window picker will appear when streaming starts."
                echo "You will select the window interactively."
            else
        ...
        3)
            CAPTURE_MODE="region"
            if [ "$display_server" = "wayland" ]; then
                echo "Use slurp to select a region..."
                CAPTURE_TARGET=$(slurp 2>/dev/null)
                if [ -z "$CAPTURE_TARGET" ]; then
                    echo "No region selected. Falling back to full screen."
                    CAPTURE_MODE="fullscreen"
                else
                    echo "Selected region: $CAPTURE_TARGET"
                fi
            else
                echo "Click and drag to select the capture region..."
                local geometry
                geometry=$(xdotool selectwindow getwindowgeometry --shell 2>/dev/null | grep -E "^(X|Y|WIDTH|HEIGHT)" | tr '\n' ' ')
                if [ -n "$geometry" ]; then
                    eval "$geometry"
                    CAPTURE_TARGET="${WIDTH}x${HEIGHT}+${X}+${Y}"
                    echo "Selected region: $CAPTURE_TARGET"
                else
...
configure_stream_wayland() {
    echo "Configuring Wayland screen capture..."
    ...
    case "$CAPTURE_MODE" in
        fullscreen)
            ...
            ;;
        window)
            echo "A window picker will appear. Select the window to capture."
            wf_args=""
            ;;
        region)
            if [ -n "$CAPTURE_TARGET" ]; then
                wf_args="-g \"$CAPTURE_TARGET\""
            fi
            ;;
    esac

    local audio_source
    audio_source=$(pactl get-default-sink 2>/dev/null).monitor || audio_source=""

    local audio_arg=""
    if [ -n "$audio_source" ]; then
        audio_arg="--audio=$audio_source"
    fi

    echo "Starting wf-recorder + FFmpeg pipeline..."
    eval wf-recorder $wf_args $audio_arg --muxer=v4l2 --codec=rawvideo --file=- 2>/dev/null | \
        ffmpeg -f rawvideo -video_size "$STREAM_RES" -framerate "$STREAM_FPS" -pixel_format bgr0 -i pipe:0 \
    ...
    if kill -0 "$stream_pid" 2>/dev/null; then
        echo "Wayland stream running (PID: $stream_pid)"
    else
        echo "Primary method failed. Trying wf-recorder native encoding..."
        configure_stream_wayland_fallback
    fi
}
...
configure_stream_wayland_fallback() {
    ...
    if [ "$CAPTURE_MODE" = "region" ] && [ -n "$CAPTURE_TARGET" ]; then
        wf_geometry="-g $CAPTURE_TARGET"
    fi

    eval wf-recorder $wf_geometry --audio -c h264_vaapi -d /dev/dri/renderD128 -f - 2>/dev/null | \
        ffmpeg -i pipe:0 \
            -c:v libx264 -b:v "${STREAM_BITRATE}k" -pix_fmt yuv420p -preset "$STREAM_PRESET" \
            -g "$((STREAM_FPS * 2))" -keyint_min "$((STREAM_FPS * 2))" \
            -c:a aac -b:a 128k \
            -f hls -hls_time 2 -hls_list_size 5 -hls_flags delete_segments+append_list \
            "$hls_output" > "$LOG_DIR/ffmpeg.log" 2>&1 &
    ...
    sleep 3
    if kill -0 "$stream_pid" 2>/dev/null; then
        echo "Wayland stream running via fallback (PID: $stream_pid)"
    else
        echo "Error: Could not start Wayland capture."
        echo "Test manually: wf-recorder -f test.mp4"
        echo "Check log: $LOG_DIR/ffmpeg.log"
        exit 1
    fi
}

configure_stream_x11() {
    echo "Configuring X11 screen capture..."
    ...
    local hls_output="${webserver_dir}/stream.m3u8"
    local ffmpeg_input
    ffmpeg_input=$(build_ffmpeg_x11_input)

    echo "Starting FFmpeg X11 capture..."
    ...
    if kill -0 "$ffmpeg_pid" 2>/dev/null; then
        echo "FFmpeg X11 stream running (PID: $ffmpeg_pid)"
    else
...
configure_multi_bitrate() {
    ...
    local ffmpeg_input
    if [ "$display_server" = "wayland" ]; then
        echo "Note: ABR with Wayland requires the primary stream as input."
        echo "Using HLS source for transcoding variants..."
        sleep 5
        ffmpeg_input="-re -i ${hls_dir}/stream.m3u8"
    else
        ffmpeg_input=$(build_ffmpeg_x11_input)
        ffmpeg_input="$ffmpeg_input -f pulse -ac 2 -i default"
    fi
...
EOF
    ...
    echo "Adaptive bitrate enabled. Master playlist: /hls/master.m3u8"
}
...
configure_rtsp() {
    ...
    if command -v mediamtx &>/dev/null; then
        ...
        if [ "$display_server" = "wayland" ]; then
            wf-recorder -c rawvideo -m rawvideo -f pipe:1 2>/dev/null | \
                gst-launch-1.0 -v fdsrc fd=0 \
                    ! rawvideoparse width=1920 height=1080 framerate=30/1 format=bgr \
                    ! videoconvert \
...
show_status() {
    ...
    echo "============================================"
    echo "  Display: $display_server | Quality: ${STREAM_RES}@${STREAM_FPS}fps"
...
# --- Main ---
echo "=== VM2TV - Linux VM Streaming (X11 + Wayland) ==="
echo ""
...
if [ "$display_server" = "unknown" ]; then
    read -rp "Could not detect display server. Use (w)ayland or (x)11? " choice
    case "$choice" in
        w|W) display_server="wayland" ;;
        *) display_server="x11" ;;
    esac
fi

check_system_resources
install_packages "$display_server"
configure_nginx
create_web_player
select_quality_preset
select_capture_source "$display_server"
